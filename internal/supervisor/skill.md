...
```bash
baton take cmp-client --wait     # blocks until the container serves YOUR tree
# ... run your tests ...
baton check cmp-client           # exit 0 = still yours, 2 = throw the results away
...
```bash
baton take cmp-client --wait --timeout 30m
```
...
If you cannot background it, `baton take cmp-client` without `--wait` returns
...
**After every test batch, run `baton check cmp-client`. If it exits 2, discard
...
```bash
baton status cmp-client    # holder, what is being served, who is waiting
baton line cmp-client      # just the queue
```
...
| `baton grab` / `baton drop` | Human override, optionally on a named worktree. **Not for you to run.** |
| `baton trees <container>` | Worktrees the container can be switched to. |
...
Source and full README: `/Users/guru.gayen/newscred/baton`
